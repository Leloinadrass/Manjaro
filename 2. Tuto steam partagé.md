...
14. Evidemment, je reprends le tuto 5 mois plus tard, et j'ai oublié : il faut redonner les droits à l'utilisateur. Lorsqu'on crée les dossiers/fichiers d'un autre utilisateur depuis le premier utilisateur, on utilise SUDO pour avoir accès à leur /home, et tout ce qu'on crée devient la propriété de ROOT.

Donc à la fin, un petit ```sudo chown -R user2:user2 /home/user2``` permettra d'éviter les conflits de droits. 
